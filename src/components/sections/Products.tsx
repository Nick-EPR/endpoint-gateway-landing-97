  ...
  return <section id="solutions" className="py-32 md:py-48">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {solutions.map((solution, index) => <div key={index} className={`solution-card p-8 glass-card rounded-xl hover:shadow-lg animate-float ${solution.comingSoon ? 'opacity-75' : ''}`} style={{
          animationDelay: `${index * 200}ms`
        }}>
              {solution.logo ? <div className="mb-6 h-16 flex items-center justify-center">
                  <img src={solution.logo} alt={`${solution.title} logo`} className={`object-contain ${solution.title === 'HeliAM' ? 'h-12' : solution.title === 'Luemin' ? 'max-w-[160px]' : 'max-h-8'}`} />
                </div> : <h3 className="text-xl font-semibold mb-6">{solution.title}</h3>}
              <p className="text-neutral mb-6">{solution.description}</p>
              {solution.comingSoon ? <div className="flex justify-center mt-4">
                  <span className="text-primary font-medium">Coming Soon</span>
                </div> : solution.path && <Link to={solution.path}>
