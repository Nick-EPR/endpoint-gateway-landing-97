...
const Features = () => {
  const features: Feature[] = [
    ...
    {
      icon: <Users className="w-8 h-8 text-primary animate-float" />,
      title: "Remote Support",
      description: "Direct device access for remote troubleshooting and maintenance",
      comingSoon: true
    },
    {
      icon: <Shield className="w-8 h-8 text-primary animate-bounce" />,
    ...
    {
      icon: <Recycle className="w-8 h-8 text-primary animate-spin-slow" />,
          ...
          {features.map((feature, index) => (
            ...
            >
              ...
              {feature.link && (
                <Link 
                  to={feature.link.to} 
                  className="inline-flex items-center text-xs text-primary hover:text-primary/80 font-medium transition-colors mt-2"
                >
                  {feature.link.text}
                  <svg 
